...
public class Exercises {
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
    	double addBirthday = mealAmount + 5.00;
    	if(isBirthday){
    		if(addBirthday <= 10) {
    			return "standard";
			} else if (addBirthday <= 15 && addBirthday > 10){
    			return "special";
			} else if(addBirthday>15) {
    			return "ginormous";
			}
		} else {
    		if(mealAmount <= 10) {
    			return "standard";
			} else if (mealAmount <= 15 && addBirthday > 10){
    			return "special";
